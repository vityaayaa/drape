...
export default function ExportDialog({ walls, tile, palette, surplusPercent, onClose }) {
  ...
  // При grout_width = 0 — отключаем швы автоматически
  useEffect(() => {
  ...
  const handleDownload = () => {
    ...
    // Генерируем в следующем тике чтобы не заблокировать UI
    setTimeout(() => {
      try {
        ...
        downloadSVG(svg, 'drape-schema.svg')
        onClose()
      } finally {
  ...
  return (
    <>
      ...
      <div style={s.sheet}>
        ...
        <p style={s.title}>Настройки экспорта</p>
          ...
          <div style={s.radioGroup}>
            ...
            <label style={s.radioLabel}>
              <input
                ...
                name="scaleMode"
                value="real"
                checked={scaleMode === 'real'}
                onChange={() => setScaleMode('real')}
                style={s.radioInput}
              />
        ...
        <div style={s.section}>
          <div style={s.toggleRow}>
            <div>
              ...
              {groutWidth === 0 && (
                <p style={s.disabledHint}>Ширина шва = 0 — швы не нужны</p>
              )}
            </div>
            <button
        ...
        <div style={s.footer}>
          ...
          {longWait && (
            <p style={s.longWaitHint}>Большой файл, ещё немного…</p>
          )}
        </div>
      </div>
...
const s = {
  ...
  spinner: {
    ...
    borderTopColor: '#fff',
    borderRadius: '50%',
    animation: '_exportSpin 0.7s linear infinite',
    flexShrink: 0,
  },
  longWaitHint: {
    textAlign: 'center',
